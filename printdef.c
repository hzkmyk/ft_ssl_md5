...
/*   Updated: 2018/11/11 17:27:31 by hmiyake          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	printfdefwitharg256(t_ssl *ssl, char **argv, int *i)
{
	if (ISSAME(*ssl->pqrs, Q))
		...
		ssl->uv[0], ssl->uv[1], ssl->uv[2], ssl->uv[3],
		ssl->uv[4], ssl->uv[5], ssl->uv[6], ssl->uv[7], argv[i[0]]);
	else
		ft_printf("sha256 (%s) = %.8x%.8x%.8x%.8x%.8x%.8x%.8x%.8x\n",
		argv[i[0]], ssl->uv[0], ssl->uv[1], ssl->uv[2], ssl->uv[3],
		ssl->uv[4], ssl->uv[5], ssl->uv[6], ssl->uv[7]);
}

void	printdefwitharg512(t_ssl *ssl, char **argv, int *i)
{
	if (ISSAME(*ssl->pqrs, Q))
		...
		"%.16llx %s\n", ssl->uv512[0], ssl->uv512[1], ssl->uv512[2],
		ssl->uv512[3], ssl->uv512[4], ssl->uv512[5], ssl->uv512[6],
		ssl->uv512[7], argv[i[0]]);
	else
		ft_printf("sha512 (%s) = %.16llx%.16llx%.16llx%.16llx%.16llx"
		"%.16llx%.16llx%.16llx\n", argv[i[0]], ssl->uv512[0], ssl->uv512[1],
		ssl->uv512[2], ssl->uv512[3], ssl->uv512[4], ssl->uv512[5],
		ssl->uv512[6], ssl->uv512[7]);
}
...
void	printdefwitharg384(t_ssl *ssl, char **argv, int *i)
{
	if (ISSAME(*ssl->pqrs, Q))
		...
		ssl->uv512[0], ssl->uv512[1], ssl->uv512[2], ssl->uv512[3],
		ssl->uv512[4], ssl->uv512[5], argv[i[0]]);
	else
		ft_printf("sha384 (%s) = %.16llx%.16llx%.16llx%.16llx"
		"%.16llx%.16llx\n", argv[i[0]], ssl->uv512[0], ssl->uv512[1],
		ssl->uv512[2], ssl->uv512[3], ssl->uv512[4], ssl->uv512[5]);
}
...
void	printfdefwitharg224(t_ssl *ssl, char **argv, int *i)
{
	if (ISSAME(*ssl->pqrs, Q))
		...
		ssl->uv[0], ssl->uv[1], ssl->uv[2], ssl->uv[3],
		ssl->uv[4], ssl->uv[5], ssl->uv[6], argv[i[0]]);
	else
		ft_printf("sha256 (%s) = %.8x%.8x%.8x%.8x%.8x%.8x%.8x\n",
		argv[i[0]], ssl->uv[0], ssl->uv[1], ssl->uv[2],
