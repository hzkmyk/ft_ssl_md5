...
/*   Updated: 2018/11/10 19:01:58 by hmiyake          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	return ((RIGHTROTATE512(1, nay[i - 15]))
	^ (RIGHTROTATE512(8, nay[i - 15])) ^ (nay[i - 15] >> 7));
}

u_int64_t	s1_512(u_int64_t *nay, int i)
{
	return ((RIGHTROTATE512(19, nay[i - 2]))
	^ (RIGHTROTATE512(61, nay[i - 2])) ^ (nay[i - 2] >> 6));
}
